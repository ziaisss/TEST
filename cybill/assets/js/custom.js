...
$(function(){
  ...
  $( ".move-down" ).on( "click", function() {
    $('html, body').animate({
      scrollTop: $('.info-sec').offset().top
    }, 700);
  }); 

  $( ".mobile-nav .drp" ).on( "click", function() {
  ...
  if ($("#progress").length > 0) {
    ...
    var $progsec = $("#progress");

    $(document).bind('scroll', function(ev) {
        var scrollOffset = $(document).scrollTop();
        var containerOffset = $progsec.offset().top - window.innerHeight;
    ...
    $("#custom-quote-form").validate({
     ...
     $("#quote-form-pg, #contact-form").validate({
      rules: {
        name: {
          required: true,
          minlength: 2
    ...
    $("#consult-form").validate({
      ...
      messages: {
        Name: "Please enter your Name",
        email: "Please enter a valid email address",
        agree: "Please hit the check box to confirm your request"        
      }
    });
    ...
    $('.portfolio-pg #portfolio-filters button:first-child').trigger('click');
    $('.home-pg #portfolio-filters button:first-child').trigger('click');
...
function progress_appear() {
    $('.progress-title strong span').each(function(){
...
$('.services').click(function(){
	$('#free_cons_home_chkbox').val(0);
	$( ".services" ).each(function( index ) {
	  if($(this).is(':checked')){
		  $('#free_cons_home_chkbox').val(1);
		  return false;
	  }
	});
});
...
function free_cons_home_form_validate(){
	$('.form_name').val('free_cons_form_home');
	/* $('#form-2').html($('#form-1').html());
	$('#form-1').html(''); */
	grecaptcha.reset(widgetId2);
	var filter = /^([\w-\.]+)@((\[[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}\.)|(([\w-]+\.)+))([a-zA-Z]{2,4}|[0-9]{1,3})(\]?)$/;
	if(!filter.test($('#free_cons_home_email').val()) && $('#free_cons_home_email').val() != ""){
		$('#free_cons_home_email').addClass('error');
		$('.free_cons_home_errors').text('Kindly provide valid email address!');
		$('.free_cons_home_errors').slideDown('slow');
		return false;
	}
	if($('#free_cons_home_name').val() == "" || $('#free_cons_home_email').val() == "" || $('#free_cons_home_phone').val() == "" ){
		if($('#free_cons_home_name').val() == "")	{ 	$('#free_cons_home_name').addClass('error'); 	}
		if($('#free_cons_home_email').val() == ""){ 	$('#free_cons_home_email').addClass('error'); 	}
		if($('#free_cons_home_phone').val() == ""){ 	$('#free_cons_home_phone').addClass('error'); 	}
		$('.free_cons_home_errors').text('Fields marked with * are required!');
		$('.free_cons_home_errors').slideDown('slow');
		return false;
	}
	if($('#free_cons_home_chkbox').val() == 0){
		$('.free_cons_home_errors').text('Kindly select service in which you are interested!');
		$('.free_cons_home_errors').slideDown('slow');
		return false;
	}
		$('.free_cons_home_errors').slideUp();
		grecaptcha.execute(widgetId2);
	
}
function floatinglpform_validate(){
    
    grecaptcha.reset(widgetId9);
    $('.form_name').val('floatinglpform');
    var filter = /^([\w-\.]+)@((\[[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}\.)|(([\w-]+\.)+))([a-zA-Z]{2,4}|[0-9]{1,3})(\]?)$/;
    if(!filter.test($('#floating_form_email').val()) && $('#floating_form_email').val() != ""){
        $('#floating_form_email').addClass('error');
        $('.floating_form_errors').text('Kindly provide valid email address!');
        $('.floating_form_errors').slideDown('slow');
        return false;
    }
    if($('#floating_form_name').val() == "" || $('#floating_form_email').val() == "" || $('#floating_form_phone').val() == "" ){
        if($('#floating_form_name').val() == "") {   $('#floating_form_name').addClass('error');  }
        if($('#floating_form_email').val() == ""){   $('#floating_form_email').addClass('error');     }
        if($('#floating_form_phone').val() == ""){   $('#floating_form_phone').addClass('error');     }
        $('.floating_form_errors').text('Fields marked with * are required!');
        $('.floating_form_errors').slideDown('slow');
    }else{
        $('#floating_form_name').removeClass('error');
        $('#floating_form_email').removeClass('error');
        $('#floating_form_phone').removeClass('error');
        $('.floating_form_errors').slideUp();
        grecaptcha.execute(widgetId9);
    }
}
function custom_quote_form_validate(){
	
	grecaptcha.reset(widgetId1);
	$('.form_name').val('custom_quote_form');
	var filter = /^([\w-\.]+)@((\[[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}\.)|(([\w-]+\.)+))([a-zA-Z]{2,4}|[0-9]{1,3})(\]?)$/;
	if(!filter.test($('#custom_quote_email').val()) && $('#custom_quote_email').val() != ""){
		$('#custom_quote_email').addClass('error');
		$('.custom_quote_errors').text('Kindly provide valid email address!');
		$('.custom_quote_errors').slideDown('slow');
		return false;
	}
	if($('#custom_quote_name').val() == "" || $('#custom_quote_email').val() == "" || $('#custom_quote_phone').val() == "" ){
		if($('#custom_quote_name').val() == "")	{ 	$('#custom_quote_name').addClass('error'); 	}
		if($('#custom_quote_email').val() == ""){ 	$('#custom_quote_email').addClass('error'); 	}
		if($('#custom_quote_phone').val() == ""){ 	$('#custom_quote_phone').addClass('error'); 	}
		$('.custom_quote_errors').text('Fields marked with * are required!');
		$('.custom_quote_errors').slideDown('slow');
	}else{
		$('#custom_quote_name').removeClass('error');
		$('#custom_quote_email').removeClass('error');
		$('#custom_quote_phone').removeClass('error');
		$('.custom_quote_errors').slideUp();
		grecaptcha.execute(widgetId1);
	}
}
function contact_us_form_validate(){
	
	grecaptcha.reset(widgetId1);
	$('.form_name').val('contact_us_form');
	var filter = /^([\w-\.]+)@((\[[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}\.)|(([\w-]+\.)+))([a-zA-Z]{2,4}|[0-9]{1,3})(\]?)$/;
	if(!filter.test($('#contact_email').val()) && $('#contact_email').val() != ""){
		$('#contact_email').addClass('error');
		$('.contact_us_errors').text('Kindly provide valid email address!');
		$('.contact_us_errors').slideDown('slow');
		return false;
	}
	if($('#contact_name').val() == "" || $('#contact_email').val() == "" || $('#contact_phone').val() == "" ){console.log
		if($('#contact_name').val() == "")	{ 	$('#contact_name').addClass('error'); 	}
		if($('#contact_email').val() == ""){ 	$('#contact_email').addClass('error'); 	}
		if($('#contact_phone').val() == ""){ 	$('#contact_phone').addClass('error'); 	}
		$('.contact_us_errors').text('Fields marked with * are required!');
		$('.contact_us_errors').slideDown('slow');
	}else{
		$('#contact_name').removeClass('error');
		$('#contact_email').removeClass('error');
		$('#contact_phone').removeClass('error');
		$('.contact_us_errors').slideUp();
		grecaptcha.execute(widgetId1);
	}
}
function free_cons_form_validate(){
	
	grecaptcha.reset(widgetId2);
	$('.form_name').val('free_cons_form');
	var filter = /^([\w-\.]+)@((\[[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}\.)|(([\w-]+\.)+))([a-zA-Z]{2,4}|[0-9]{1,3})(\]?)$/;
	if(!filter.test($('#free_cons_email').val()) && $('#free_cons_email').val() != ""){
		$('#free_cons_email').addClass('error');
		$('.free_cons_errors').text('Kindly provide valid email address!');
		$('.free_cons_errors').slideDown('slow');
		return false;
	}
	if($('#free_cons_name').val() == "" || $('#free_cons_email').val() == "" || $('#free_cons_phone').val() == "" ){console.log
		if($('#free_cons_name').val() == "")	{ 	$('#free_cons_name').addClass('error'); 	}
		if($('#free_cons_email').val() == ""){ 	$('#free_cons_email').addClass('error'); 	}
		if($('#free_cons_phone').val() == ""){ 	$('#free_cons_phone').addClass('error'); 	}
		$('.free_cons_errors').text('Fields marked with * are required!');
		$('.free_cons_errors').slideDown('slow');
	}else{
		$('#free_cons_name').removeClass('error');
		$('#free_cons_email').removeClass('error');
		$('#free_cons_phone').removeClass('error');
		$('.free_cons_errors').slideUp();
		grecaptcha.execute(widgetId2);
	}
}

function free_cons_form_validate1(){
	
	grecaptcha.reset(widgetId5);
	$('.form_name').val('order_now_form');
	var filter = /^([\w-\.]+)@((\[[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}\.)|(([\w-]+\.)+))([a-zA-Z]{2,4}|[0-9]{1,3})(\]?)$/;
	if(!filter.test($('#free_cons_email1').val()) && $('#free_cons_email1').val() != ""){
		$('#free_cons_email1').addClass('error');
		$('.free_cons_errors').text('Kindly provide valid email address!');
		$('.free_cons_errors').slideDown('slow');
		return false;
	}
	if($('#free_cons_name1').val() == "" || $('#free_cons_email1').val() == "" || $('#free_cons_phone1').val() == "" ){
		if($('#free_cons_name1').val() == "")	{ 	$('#free_cons_name1').addClass('error'); 	}
		if($('#free_cons_email1').val() == ""){ 	$('#free_cons_email1').addClass('error'); 	}
		if($('#free_cons_phone1').val() == ""){ 	$('#free_cons_phone1').addClass('error'); 	}
		$('.free_cons_errors').text('Fields marked with * are required!');
		$('.free_cons_errors').slideDown('slow');
	}else{
		$('#free_cons_name1').removeClass('error');
		$('#free_cons_email1').removeClass('error');
		$('#free_cons_phone1').removeClass('error');
		$('.free_cons_errors').slideUp();
		grecaptcha.execute(widgetId5);
	}
}
function get_a_free_quote_form_validate(){
	$('.form_name').val('get_free_quote_form');
	/* $('#form-2').html($('#form-1').html());
	$('#form-1').html(''); */
	grecaptcha.reset(widgetId2);
	var filter = /^([\w-\.]+)@((\[[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}\.)|(([\w-]+\.)+))([a-zA-Z]{2,4}|[0-9]{1,3})(\]?)$/;
	if(!filter.test($('#get_free_quote_email').val()) && $('#get_free_quote_email').val() != ""){
		$('#get_free_quote_email').addClass('error');
		$('.get_free_quote_errors').text('Kindly provide valid email address!');
		$('.get_free_quote_errors').slideDown('slow');
		return false;
	}
	if($('#get_free_quote_name').val() == "" || $('#get_free_quote_email').val() == "" || $('#get_free_quote_phone').val() == "" ){
		if($('#get_free_quote_name').val() == "")	{ 	$('#get_free_quote_name').addClass('error'); 	}
		if($('#get_free_quote_email').val() == ""){ 	$('#get_free_quote_email').addClass('error'); 	}
		if($('#get_free_quote_phone').val() == ""){ 	$('#get_free_quote_phone').addClass('error'); 	}
		$('.get_free_quote_errors').text('Fields marked with * are required!');
		$('.get_free_quote_errors').slideDown('slow');
		return false;
	}
	if($('#free_cons_home_chkbox').val() == 0){
		$('.get_free_quote_errors').text('Kindly select service in which you are interested!');
		$('.get_free_quote_errors').slideDown('slow');
		return false;
	}
		$('.get_free_quote_errors').slideUp();
		grecaptcha.execute(widgetId2);
	
}
function custom_quote_form(){
	if($('.form_name').val() == "free_cons_form_home"){
		$('#consult-form').submit();
		console.log('p cons form submittion here!');
	}else if($('.form_name').val() == "custom_quote_form"){
		$('#custom-quote-form').submit();
		console.log('custom form submittion here!');
	}else if($('.form_name').val() == "contact_us_form"){
		$('#contact-form').submit();
		console.log('contact-form submittion here!');
	}else if($('.form_name').val() == "free_cons_form"){
		$('#consult-form').submit();
		console.log('contact-form submittion here!');
	}else if($('.form_name').val() == "get_free_quote_form"){
		$('#quote-form-pg').submit();
		console.log('contact-form submittion here!');
	}
	else if($('.form_name').val() == "order_now_form"){
		$('#ordernow-form').submit();
		console.log('ordernow-form submittion here!');
	}
	else if($('.form_name').val() == "floatinglpform"){
		$('#floating-lp-form').submit();
		console.log('floating-lp-form submittion here!');
	}
...
function gotoscroll(getid){
	$('html,body').animate({scrollTop: $(getid).offset().top - 50},'slow');	
}
